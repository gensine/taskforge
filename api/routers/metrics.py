...
async def recent_jobs(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def pause_queue(queue_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def resume_queue(queue_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await db.execute(text("UPDATE queues SET is_paused = false WHERE id = :qid"), {"qid": str(queue_id)})
    await db.commit()
    return {"status": "resumed"}

@router.delete("/queues/{queue_id}")
async def delete_queue(queue_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
